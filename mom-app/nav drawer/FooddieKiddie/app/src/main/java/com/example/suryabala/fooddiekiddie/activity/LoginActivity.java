...
public class LoginActivity extends AppCompatActivity {
    ...
    public void onLogin(View view) throws ExecutionException, InterruptedException, JSONException {
        ...
        String result =  new BackgroundWorker(LoginActivity.this).execute(postData,url).get() ;
        JSONObject json_data= new JSONObject(result);
        String data = json_data.getString("data");
        if(data.equals("failed")){
            Toast.makeText(LoginActivity.this,"Invalid password or email address",Toast.LENGTH_LONG).show();
        }else {
